...
class ChatListTableViewController: UITableViewController {
    ...
    var popUpSettings = PopUpSettings(
        ...
        contentLabelText: """
                        ...
                        ②各行を左にスワイプして非表示ボタンをタップすると、メッセージが非表示になります。再表示する場合は、「いいね」画面から該当ユーザーのチャット画面に遷移し、新しいメッセージを送信するとメッセージが再表示されます。「いいね」画面にユーザーがいない場合、相手からチャットが来ない限りメッセージは再表示されません。
                        
                        ③各行を左にスワイプしてブロックボタンをタップすると、ユーザーをブロックでき、メッセージ画面といいね画面からユーザーが表示されなくなります。ブロックしたユーザーがメッセージを送信しても表示されません。
                        
                        ④各行を右にスワイプして報告ボタンをタップすると、ユーザーを報告できます。報告内容を確認し、運営側で厳正な対処を行います。
                        """,
        popupViewHeight: 530
    )
